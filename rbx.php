<?php
// 2017-6 ns added first version of custom id fields
...
class OODBBean implements\IteratorAggregate,\ArrayAccess,\Countable,Jsonable
{
	...
	{
		$adapter= $beanhelper->getToolbox();
		$adapter= $adapter->getRedBean();
		$idField= $adapter->getIdField($type);
		$this->beanHelper         = $beanhelper;
		$this->__info['type']     = $type;
		$this->__info['sys.id']   = $idField;// niek 'id';		// not sure were used for?!
		$this->__info['sys.idfield']= $idField; // niek
		$this->__info['sys.orig'] = array($this->__info['sys.idfield'] => 0 );
		$this->__info['tainted']  = TRUE;
		$this->__info['changed']  = TRUE;
		$this->properties[$this->__info['sys.idfield']]= 0;
	}
	...
	*/
	...
	{
		$myID = $this->properties[$this->__info['sys.idfield']]; // niek 
	...
	{
		return @$this->properties[$this->__info['sys.idfield']];
		// return ( isset( $this->properties[$this->__info['sys.idfield']] ) ) ? (string) $this->properties[$this->__info['sys.idfield']] : NULL;
	}


	public function setID($value)
	{
		$this->properties[$this->__info['sys.idfield']]= $value;
	}
	/**
		...
		if ( isset( $this->$fieldLink ) ) {
			...
			if ( isset( $this->__info["sys.parentcache.$property"] ) ) {
				...
				if ( !is_null( $this->properties[$fieldLink] ) ) {
					...
					if ( $bean->getID() !== $this->properties[$fieldLink] && self::$autoResolve ) {
	...
	public function isEmpty()
	{
		...
		foreach ( $this->properties as $key => $value ) {
			if ( $key == $this->__info['sys.idfield'] ) {
				continue;
			}
			if ( !empty( $value ) ) {
				$empty = FALSE;
	...
	public function one( $type ) {
		return $this->beanHelper->getToolBox()->getRedBean()->load( $type, $this->getID() );
	}
	...
	{
		return $this->beanHelper->getToolbox()->getRedBean()->load( $this->getMeta( 'type' ), $this->properties[$this->__info['sys.idfield']] );
	}
	...
	public function &aggr( $list, $property, $type = NULL )
	{
		...
		foreach( $beans as $bean ) {
			$beanIndex[$bean->getID()]->setProperty( $property, $bean );
		}
	...
	{
		return (bool) (
			   ( (string) $this->getID() === (string) $bean->getID() )
...
class DBAdapter extends Observable implements Adapter
{
	...
	*/
	 ...
	 */
	public function __construct( $database, $oodb )
	{
		$this->db = $database;
		$this->oodb= $oodb;
	}
...
abstract class AQueryWriter
{
	...
	protected function insertRecord( $type, $insertcolumns, $insertvalues )
	{
		...
		$table   = $this->esc( $type );
		$idfield = $this->oodb->getIdField($type);
	...
	public function updateRecord( $type, $updatevalues, $id = NULL )
	{
		...
		$sql .= implode( ',', $p ) . ' WHERE '.$this->adapter->getIdField($type).' = ? ';
...
class MySQL extends AQueryWriter implements QueryWriter
{
	...
	public function createTable( $table )
	{
		...
		$sql   = "CREATE TABLE $table ($idfield INT( 11 ) UNSIGNED NOT NULL AUTO_INCREMENT, PRIMARY KEY ( $idfield )) ENGINE = InnoDB DEFAULT CHARSET={$encoding} COLLATE={$encoding}_unicode_ci ";
...
abstract class Repository
{
	...
	*/
	...
	// use $this->oodb->getIdField()
	public function getIdField_($table)
	{
		return $this->oodb->getIdField();
		if(isset($this->idFieldMap[$table])) return $this->idFieldMap[$table];
		else {
			return 'id';
		}
	}
	...
	{
		if ( !$embeddedBean->getID() || $embeddedBean->getMeta( 'tainted' ) ) {
			$this->store( $embeddedBean );
		}

		return $embeddedBean->getID();
	}
	...
	public function store( $bean )
	{
		...
		return ( (string) $bean->getID() === (string) (int) $bean->getID() ) ? (int) $bean->getID() : (string) $bean->getID();
	}
	...
	public function trash( $bean )
	{
		...
		try {
			$this->writer->deleteRecord( $bean->getMeta( 'type' ), array( $bean->getMeta( 'sys.idfield' ) => array( $bean->getID() ) ), NULL );
		} catch ( SQLException $exception ) {
			$this->handleException( $exception );
		}
		$bean->setID(0);
...
class Fluid extends Repository
{
	...
	protected function processAdditions( $bean, $ownAdditions )
	{
		...
		foreach ( $ownAdditions as $addition ) {
			if ( $addition instanceof OODBBean ) {
				...
				$addition->$myFieldLink = $bean->getID();
	...
	protected function storeBean( OODBBean $bean )
	{
		if ( $bean->getMeta( 'changed' ) ) {
			...
			$updateValues = array();
			foreach ( $bean as $property => $value ) {
				if ( $property !== $bean->getMeta('sys.idfield') ) {
					$this->modifySchema( $bean, $property, $value );
					$updateValues[] = array( 'property' => $property, 'value' => $value );
				}
			}

			$bean->setID( $this->writer->updateRecord( $table, $updateValues, $bean->getID() ) );
	...
	public function load( $type, $id )
	{
		...
		if ( isset( $this->stash[$this->nesting][$id] ) ) {
			...
			try {
				$idField= $this->oodb->getIdField($type);
				$rows = $this->writer->queryRecord( $type, array( $idField => array( $id ) ) );
			} catch ( SQLException $exception ) {
				if ( $this->writer->sqlStateIn( $exception->getSQLState(),
					array(
						QueryWriter::C_SQLSTATE_NO_SUCH_COLUMN,
...
class Frozen extends Repository
{
	...
	protected function storeBean( OODBBean $bean )
	{
		if ( $bean->getMeta( 'changed' ) ) {
			...
			$id = $bean->getID();// $properties['id'];
			nounset($properties['id']);
			$updateValues = array();
			$k1 = 'property';
			$k2 = 'value';
			...
			$bean->setID( $this->writer->updateRecord( $table, $updateValues, $id ));
			$bean->setMeta( 'changed', FALSE );
		}
		$bean->setMeta( 'tainted', FALSE );
	}
...
class OODB extends Observable
{
	...
	/** 
	* An array with custom id fieldnames (to support existing database)
...
class AssociationManager extends Observable
{
	...
	protected function associateBeans( OODBBean $bean1, OODBBean $bean2, OODBBean $bean )
	{
		...
		$bean->$property1 = $bean1->getID();
		$bean->$property2 = $bean2->getID();
	...
	public function relatedCount( $bean, $type, $sql = NULL, $bindings = array() )
	{
		...
		try {
			return $this->writer->queryRecordCountRelated( $beanType, $type, $bean->getID(), $sql, $bindings );
		} catch ( SQLException $exception ) {
			$this->handleException( $exception );
		...
		foreach ( $beans1 as $bean1 ) {
			foreach ( $beans2 as $bean2 ) {
				try {
					...
					$row      = $this->writer->queryRecordLink( $type1, $type2, $bean1->getID(), $bean2->getID() );
					$linkType = $this->getTable( array( $type1, $type2 ) );

					if ( $fast ) {
						$idField= $this->adapter->getIdField($linkType);
						$this->writer->deleteRecord( $linkType, array( $idField => $row[$idField] ) );
						return;
					}
	...
	public function clearRelations( OODBBean $bean, $type )
	{
		...
		try {
			$this->writer->deleteRelations( $bean->getMeta( 'type' ), $type, $bean->getID() );
		} catch ( SQLException $exception ) {
			$this->handleException( $exception );
		}
	}
	...
	public function related( $bean, $type, $sql = '', $bindings = array() )
	{
		...
		foreach ( $beans as $bean ) $bean->setMeta( 'sys.belongs-to', $links[$bean->getID()] );
...
class Facade
{
	...
	* @return
	*/
	//public static function getIdField($type,$db=null){ return isset(self::$idfieldmap[$type]) ? self::$idfieldmap[$type] : 'id'; }
...
class DuplicationManager
{
	...
	private function createCopy( OODBBean $bean )
	{
		...
		$copy->setMeta( 'sys.dup-from-id', $bean->getID() );
		$copy->setMeta( 'sys.old-id', $bean->getID() );
	...
	protected function duplicate( OODBBean $bean, $trail = array(), $preserveIDs = FALSE )
	{
		...
		$copy->id = ( $preserveIDs ) ? $bean->getID() : $copy->getID();
...
if (!function_exists('EID')) {
	...
	{
		return \RedBeanPHP\Facade::enum( $enumName )->getID();
	}
